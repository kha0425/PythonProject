...
toggle = st.toggle('Turn on the switch', value = True) # value = True -> 초기화
if toggle:
    st.write('Switch is turned on!')
else:
    st.write('Stitch is turned off!')
...
option3 = st.radio(
    'What is your favoritemovie genre',
    ['Comdey','Drama','Document'],
    captions = ['Lauph out loud','Get the popcorn', 'Naver stop learning'] # 보충 설명
)
if option3:
...
string = st.text_input(
    'Movie title',
    placeholder='write down the title of you favorite movie',
    max_chars=32
)
if string:
...
file = st.file_uploader(
    'Choose a file',
    type = 'csv',
    accept_multiple_files=False # 동시선택 기능
)
if file is not None:
    df = pd.read_csv(file)
    st.write(df)
